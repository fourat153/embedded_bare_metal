...
int main(void)
{
    ...
    while (1)
    {
        if (((GPIOA_IDR & (1 << 1)) >> 1) == 0)
        {
            ...
            while (((GPIOA_IDR & (1 << 1)) >> 1) == 0); // go to position n and it take result return to n position compared to 0 
        }

        if (((GPIOA_IDR & (1 << 13)) >> 13) == 1)// go to position n and it take result return to n position compared to 1 
        {
